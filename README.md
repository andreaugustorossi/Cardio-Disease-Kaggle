...
This project was based on the Kaggle's dataset that provides information on 70.000 patients with and without cardiovascular diseases. To make it more interesting, a hypothetical industry-driven situation was formulated by Meigaron Lopes. The hypothetical company called Cardio Catch Diseases is a company specialized in cardiovascular diagnoses and its profits come from the diagnoses accuracy. The current diagnose is made via software or by experts, and the accuracy achieved ranges from 55% to 65%. The cost per patient is $1000,00. The price paid by the patient for the diagnosis varies according to the precision of the diagnosis. For every 1% of precision above 50% the patient pays $100,00. For instance, if a patient is diagnosed with 55% of accuracy, the patient will pay $500,00. If the accuracy is 50%, the patient does not pay anything. 
...
The current diagnostic method has caused the company to lose money and make a very low profit, because if the accuracy is set at 55% it can cause a  $35.000.000,00 debt. Thus, this project aimed to solve this problem by improving the diagnostic accuracy.
...
- The average age is 52 years, ranging from 29 to 64;
...
- Height - negative moderated skewed;
...
#### The decision taken was to remove the outliers from the dataset according to medical literature. In total, only 2.02% of the data were removed.
...
#### Based on medical literature, the follow hypothesis were formulated:
...
Two hyperparameter fine tuning technics were applied, Bayes Search and Grid Search. The results were the same. The tuned model ended up showing a slightly better performance compared to the default model in terms of precision. An attempt was made to improve the model using isotonic and sigmoid calibration, but there was no improvement.
...
The CatBoost classifier generated CVD forecasts with average accuracy of 75.5%. The entire data set has 70,000 patients. To calculate the profit generated by the model, the outcomes of the current solution (health software) were considered and compared to the model.
...
The current solution (health software) can generate a terrible outcome of **$35 million debt**. Even in the worst scenario the Machine Learning Model will never generate a debt. In that case, the outcome still remains positive, resulting in a **$105 million profit.**
